...
public class TargetRetryPolicy {
  ...
  public <T> RetryPolicy<T> get(
    ConsumerRecord<String, String> record,
    final ToIntFunction<T> getStatusCode
  ) {
      ...
      .handleResultIf(
        r ->
          String
            ...
            .matches(Config.RETRY_PROCESS_WHEN_STATUS_CODE_MATCH)
      )
      .onSuccess(
        x -> {
          var statusCode = String.valueOf(
            getStatusCode.applyAsInt(x.getResult())
          );

          if (
            statusCode.matches(
              Config.PRODUCE_TO_RETRY_TOPIC_WHEN_STATUS_CODE_MATCH
            )
          ) {
          ...
          if (
            statusCode.matches(
              Config.PRODUCE_TO_DEAD_LETTER_TOPIC_WHEN_STATUS_CODE_MATCH
            )
          ) {
            Monitor.processMessageError();
        ...
        x ->
          Monitor.targetExecutionRetry(
            ...
            Optional.<String>ofNullable(
              String.valueOf(getStatusCode.applyAsInt(x.getLastResult()))
            ),
            x.getLastFailure(),
            x.getAttemptCount()
          )
