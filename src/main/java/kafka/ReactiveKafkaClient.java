...
public class ReactiveKafkaClient<K, V>
  extends Flux<ConsumerRecords<K, V>>
  ...
  class CloseEvent implements Runnable {
    ...
    boolean await() {
      ...
      long remainingNanos;
      while (
        !closed && (remainingNanos = closeEndTimeNanos - System.nanoTime()) > 0
      ) {
        try {
  ...
  static final class EventThreadFactory implements ThreadFactory {
    ...
    public final Thread newThread(Runnable runnable) {
      String newThreadName =
        PREFIX + groupId + "-" + COUNTER_REFERENCE.incrementAndGet();
